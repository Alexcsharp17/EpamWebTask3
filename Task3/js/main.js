...
for(let i = 0; i<datarr.length;i++)
{
	...
	bar.appendChild(namecont);
	namecont.className+="namecont";
	let name = document.createTextNode(datarr[i].name);
	...
	let rating = document.createTextNode(datarr[i].ratingRevievs);
	bar.appendChild(rating);
	let price = document.createElement('div');

if(datarr[i].price.oldUan==undefined){
	let newUan= document.createElement('div');
	newUan.textContent=datarr[i].price;

	price.appendChild(newUan);
	
}
else{
	let newUan= document.createElement('div');
	let oldUan= document.createElement('div');
	oldUan.textContent=datarr[i].price.oldUan;
	newUan.textContent=datarr[i].price.newUan;
	price.appendChild(oldUan);
	price.appendChild(newUan);
	newUan.className+=("newUan");
	oldUan.className+=("oldUan");
